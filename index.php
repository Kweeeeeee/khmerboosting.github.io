  ...
  if( route(0) != "admin" && route(0) != "ajax_data" ){
	...
	if( $_SESSION["msmbilisim_userlogin"] != 1  || $user["client_type"] == 1  ){
      ...
      array(
            ...
            'user'=>$user,'data'=>$_SESSION["data"],'settings'=>$settings,'search'=>urldecode($_GET["search"]),"active_menu"=>$active_menu,'ticketList'=>$ticketList,'messageList'=>$messageList,
            'ticketCount'=>new_ticket($user['client_id']),'paymentsList'=>$methodList,'transactions'=>$transaction_logs,'PaytmQR'=>$PaytmQR["method_type"],'PaytmQRimage'=>$PaytmQRimage,'bankPayment'=>$bankPayment["method_type"],'bankList'=>$bankList,
	 ...
	 echo $twig->render( $templateDir.'.twig',
      array(
            'site'=>[
                      ...
                      "description"=>$settings["site_description"],'languages'=>$languagesL,"dripfeedcount"=>$dripfeedcount,"subscriptionscount"=>$subscriptionscount
                    ],
            'styleList'=>$stylesheet["stylesheets"],'scriptList'=>$stylesheet["scripts"],'captchaKey'=>$settings["recaptcha_key"],'captcha'=>$captcha,'resetPage'=>$resetPage,'serviceCategory'=>$categories,'categories'=>$categories,
            'error'=>$error,'errorText'=>$errorText,'success'=>$success,"servicesPage"=>$serviceList,"resetType"=>$resetType,'successText'=>$successText,'title'=>$title,
            'user'=>$user,'data'=>$_SESSION["data"],'statu'=>$statusu,'settings'=>$settings,'search'=>urldecode($_GET["search"]),"active_menu"=>$active_menu,'ticketList'=>$ticketList,'messageList'=>$messageList,
            'ticketCount'=>new_ticket($user['client_id']),'paymentsList'=>$methodList,'transactions'=>$transaction_logs,'PaytmQRimage'=>$PaytmQRimage,'PaytmQR'=>$PaytmQR["method_type"],'bankPayment'=>$bankPayment["method_type"],'bankList'=>$bankList,
            'status'=>$route[1],'orders'=>$ordersList,'pagination'=>$paginationArr,'contentText'=>$content,'headerCode'=>$settings["custom_header"],
            'footerCode'=>$settings["custom_footer"],'lang'=>$languageArray,'timezones'=>$timezones
      )
    );
